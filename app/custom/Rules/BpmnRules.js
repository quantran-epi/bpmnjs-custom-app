...
BpmnRules.prototype.init = function () {
    ...
    function checkCycleFlow(source, target) {
        return recursiveCheckCycleFlow(source.incoming, target);
    }

    function recursiveCheckCycleFlow(incomings, target) {
        if (incomings.some(connection => connection.source.id === target.id)) return true;
        return incomings.some(connection => recursiveCheckCycleFlow(connection.source.incoming, target));
    }
    ...
    this.addRule('connection.create', function (context) {
        ...
        try {
            let cycle = checkCycleFlow(source, target);
            console.log('cycle', cycle);
            return !cycle
                && !source.outgoing.some(connection => connection.target?.id === target.id)
                && !target.outgoing.some(connection => connection.target?.id === source.id)
                && (source.id !== target.id)
